...
const editATask = function (
  ...
  taskEditor.value = tasks[tasks.length - 1].innerText;
  const btnEdit =
    listEditorWrapper[listEditorWrapper.length - 1].querySelector(".btn-edit");
  btnEdit.addEventListener("click", function () {
    tasks[tasks.length - 1].innerText = taskEditor.value;
    listEditorWrapper[listEditorWrapper.length - 1].style.display = "none";
    listTaskWrapper[listTaskWrapper.length - 1].style.display = "flex";
  });
};
...
const addNewTask = function () {
  if (checkTaskBeforeAdd(newTask.value)) {
    ...
    editATask(listEditTask, listEditorWrapper, listTaskWrapper, tasks);
  }
};
