...
speechRecognition.addEventListener("start", function (e) {
  ...
  valueContainer.classList.remove("done");
  valueContainer.style.visibility = "hidden";
  request.classList.remove("done");
  let html = `<p>Tell me your request</p>`;
...
speechRecognition.addEventListener("end", function () {
  ...
  btnSpeak.innerText = "Click here to speak";
});
...
const doingTask = (value) => {
  ...
  if (value.match(/Bài hát/gi)) {
    ...
    flag = true;
    let regex = value.match(/(Bài hát) (<song>.+)/i);
    console.log(regex);
    let song = regex.groups.song.replaceAll(" ", "%20").replaceAll("/", "%2F");
    href = "https://zingmp3.vn/tim-kiem/tat-ca?q=";
    href += song;
  }
  if (value.match(/video/gi)) {
    task = "I'm searching this video for you";
    flag = true;
    let regex = value.match(/(Video)\s(?<video>.+)/i);
    console.log(regex);
    let video = regex.groups.video
      .replaceAll(" ", "%20")
      .replaceAll("/", "%2F");
  ...
  if (href) {
    setTimeout(function () {
      showInformation("Done! Hope you like it!");
      request.classList.add("done");
      valueContainer.classList.add("done");
      windowOpen(href);
    }, 1500);
  }
};
