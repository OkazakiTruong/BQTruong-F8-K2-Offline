...
//Hàm sửa task
const editATask = function (listBtnEdit, listEditorWrapper, listThingTodoItem) {
  listBtnEdit[listBtnEdit.length - 1].addEventListener("click", function () {
    listEditorWrapper[listEditorWrapper.length - 1].style.display = "block";
    listThingTodoItem[listThingTodoItem.length - 1].style.display = "none";
  });
};
...
const addNewTask = function () {
  ...
  if (checkTaskBeforeAdd(newTask.value)) {
    ...
    editATask(listBtnEdit, listEditorWrapper, listThingTodoItem);
  }
};
