...
document.addEventListener("mousemove", function (e) {
  if (isMountDown) {
    ...
    console.log(-(currentPosition * itemWidth - positionChange));
    console.log(positionChange);
    if (positionChange / itemWidth <= -0.15) {
      isMountDown = false;
      changeCarouselNext();
    }
    if (positionChange / itemWidth >= 0.15) {
...
document.addEventListener("mouseup", function (e) {
  isMountDown = false;
  carouselInner.style.cursor = "inherit";

  let positionChange = e.clientX - firstCarouselClientX;
  if (positionChange / itemWidth > -0.15 || positionChange / itemWidth < 0.15) {
    changePosition();
  }
});
