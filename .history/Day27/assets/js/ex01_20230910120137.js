...
const updateCart = function () {
  if (Number(totalNumber.innerText) === 0) {
    ...
    const p = tableCartProduct.querySelector(p);
    tableCartProduct.remove(p);
  }
};
...
const AddNewProduct = function (index) {
  ...
  const numberProc = document.createElement("input");
  numberProc.type = "number";
  numberProc.value = productNumbers[index].value;
  td4.append(numberProc);
