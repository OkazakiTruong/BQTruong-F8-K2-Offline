...
const searchTask = function () {
  ...
  inputSearchTask.addEventListener("keyup", function () {
    ...
    if (inputSearchTask.value === "") {
      ...
      let html = Array.from(taskContents).map((taskContent) => {
        let regexp = new RegExp(`ReGeX${inputSearchTask.value}ReGeX`, "gi");
        console.log(
          taskContent.textContent.match(/`${inputSearchTask.value}`/)
        );
      });
    }
  });
