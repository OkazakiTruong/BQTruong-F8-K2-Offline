...
const renderLink = (contents) => {
  Array.from(contents).forEach((content) => {
    console.log("goi");
    const regex =
      /((?<url>((?:http|https)?:\/\/[-_\w\.]*[-_\w\.]+\.([a-z]{2,})(?::\d{2,})?(\/?|\/[\w\-\/?=&+#\.]))(([\w\?\=\.]+))?)|((?<phoneNumber>)(0|\+84)\d{1,9})|((?<email>)([a-z0-9]+)@([a-z]+.)([a-z]{2,})))/gi;
    const results = content.innerText.match(regex);
    console.log(content.innerText.match(regex));
    if (results) {
      let kq = content.innerText;
      results.forEach((result) => {
        result.innerHTML = result.innerText.replace(result, `<a>${result}</a>`);
      });
    }
  });
};
