...
const refreshToken = async () => {
  const refreshToken = localStorage.getItem("login_token");
  let tokens = requestRefresh(JSON.parse(refreshToken)["refreshToken"]);
  console.log(tokens);
};
const handlePost = async (data) => {
  ...
  // const refreshToken = localStorage.getItem("login_token");
  // let tokens = requestRefresh(JSON.parse(refreshToken)["refreshToken"]);
  console.log(JSON.parse(refreshToken)["refreshToken"]);
};
...
function renderFormPost() {
  ...
  const btnPost = document.querySelector(".header .btn-post-blog");
