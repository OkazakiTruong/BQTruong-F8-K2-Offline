...
const renderFormPost = (data) => {
  ...
  <h2 style = "color:white">Hãy đăng gì đó nào !</h2>
    <label>
...
function renderBlogs(blogs) {
  ...
  if (blogs) {
    ...
    <div class="blog-tags">
      ...
      <div class="blog-tag-item"># <a>${blog.userId.name}</a></div>
    </div>
  </div>
  `;
      })
...
const renderLink = (contents) => {
  Array.from(contents).forEach((content) => {
    // console.log(content);
    const regex =
      /(?:http|https):\/\/[-_\w\.]*[-_\w\.]+\.([a-z]{2,})(?::\d{2,})?(\/?|\/[\w\-\/?=&+#\.])([\w\?\=]+)/gi;
    const results = content.innerText.match(regex);
    if (results) {
      // console.log(results);
      results.forEach((result) => {
        let regex =
          /((?:http|https):\/\/[-_\w\.]*[-_\w\.]+\.(?:[a-z]{2,})(?::\d{2,})?(?:\/?|\/[\w\-\/?=&+#\.]))(([\w\?\=]+))/;
        let linkResult = result.match(regex);
        let linkYoutubeId = linkResult[2].match(/(?:\?v=)(.+)/);
        console.log(linkYoutubeId);

        result = result.replace(linkResult[1], `${linkResult[1]}embed/`);
        result = result.replace(linkResult[2], linkYoutubeId[1]);
