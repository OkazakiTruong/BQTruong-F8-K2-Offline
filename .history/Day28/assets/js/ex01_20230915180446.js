...
console.log(
  lyric.data.sentences[0].words[lyric.data.sentences[0].words.length - 1]
    .endTime
);

const lyricSentences = [];
const lyricSentencesObj = function (startTime, lyricSentence, stopTime) {
...
for (let i = 0; i < lyric.data.sentences.length; i++) {
  ...
  let stopTime =
    lyric.data.sentences[i].words[lyric.data.sentences[0].words.length - 1]
      .endTime;
  for (let j = 0; j < lyric.data.sentences[i].words.length; j++) {
    sentence += lyric.data.sentences[i].words[j].data + " ";
  }
  lyricSentences.push(startTime, sentence.trim(), stopTime);
}
