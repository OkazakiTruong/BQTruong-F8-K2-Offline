...
const loadLyric = function () {
  let index = lyric.findIndex(function (sentence) {
    console.log(sentence.words[sentence.words.length - 1].endTime);
    return (
      sentence.words[0].startTime <= audio.currentTime &&
      sentence.words[sentence.words.length - 1].endTime >= audio.currentTime
    );
  });
  // console.log(index);
};
