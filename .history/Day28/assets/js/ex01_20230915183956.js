...
const loadLyric = function () {
  ...
  for (let i = 0; i < lyricSentences.length; i++) {
    if (
      lyricSentences[i].startTime <= audio.currentTime &&
      lyricSentences[i].stopTime >= audio.currentTime
    ) {
      isShow = true;
      console.log(lyricSentences[i].lyricSentence);
    }
    if (!isShow) {
      html = `<p>Tên bài hát: Slowly</p>
      <p>Tên ca sĩ: Thịnh Suy</p>
      `;
    } else {
