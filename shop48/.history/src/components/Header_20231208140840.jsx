...
export default function Header() {
  ...
  const handleOpenMenu = () => {
    ...
    const currentHeight = menuElement.clientHeight;

    // Set an initial height if the element is currently hidden
    menuElement.style.height =
      currentHeight === 0 ? `${menuElement.scrollHeight}px` : "0";

    // Toggle the class after a short delay to allow for the initial height to be set
    setTimeout(() => {
      menuElement.classList.toggle("h-fit");
    }, 10);
  };
  return (
    <header>
      <nav className="max-w-screen-md fixed inset-x-0 bg-black bg-opacity-70 p-6 text-white flex flex-wrap justify-between items-center select-none md:flex-nowrap md:max-w-full">
        ...
        <ul
          className={
            "h-0 flex overflow-y-hidden transition-[height] duration-300 flex-col order-4 text-lg gap-x-4 w-full md:flex md:flex-row md:order-1 md:w-f md:items-center md:justify-center md:flex md:h-fit"
          }
          ref={menuRef}
        >
