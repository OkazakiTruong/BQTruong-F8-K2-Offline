...
export default function Header() {
  ...
  const handleOpenMenu = () => {
    if (menuRef.current.classList.contains("max-h-fit")) {
      menuRef.current.classList.remove("max-h-fit");
    } else {
  ...
  return (
    <header>
      <nav className="max-w-screen-md fixed inset-x-0 bg-black bg-opacity-70 p-4 pl-10 pr-10 text-white flex flex-wrap justify-between items-center select-none md:flex-nowrap md:max-w-full z-10">
        <button
          type="button"
        ...
        <ul
          className={
            "max-h-0 flex overflow-hidden transition-all duration-300 flex-col order-4 text-lg gap-x-4 w-full md:flex md:flex-row md:order-1 md:w-f md:items-center md:justify-center md:flex md:max-h-fit"
          }
          ref={menuRef}
        >
        ...
        <div
          className="absolute bottom-0 translate-y-[100.1%] left-0 w-full p-2  bg-black bg-opacity-70 flex items-center gap-x-5 justify-center hidden"
          ref={searchRef}
        >
          <input
            type="text"
            className="p-3 w-[90%] text-black bg-white"
