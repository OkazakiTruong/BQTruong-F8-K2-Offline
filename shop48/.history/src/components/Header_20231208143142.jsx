...
export default function Header() {
  ...
  const handleOpenMenu = () => {
    if (menuRef.current.classList.contains("max-h-fit")) {
      ...
      menuRef.current.style.maxHeight = "fit-content";
  ...
  return (
    <header>
      <nav className="max-w-screen-md fixed inset-x-0 bg-black bg-opacity-70 p-6 text-white flex flex-wrap justify-between items-center select-none md:flex-nowrap md:max-w-full">
        ...
        <ul
          className={
            "max-h-0 flex overflow-hidden transition-[max-height] duration-300 flex-col order-4 text-lg gap-x-4 w-full md:flex md:flex-row md:order-1 md:w-f md:items-center md:justify-center md:flex md:max-h-fit"
          }
          ref={menuRef}
        >
